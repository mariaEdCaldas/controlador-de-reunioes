/**
 * Carrega a carga real de coordenadores (`npm run carga`).
 *
 * Idempotente: por padrao so popula se ainda nao houver coordenadores.
 * Use `npm run carga -- --force` para apagar TODOS e recarregar do zero.
 */
...
if (r.pulou) {
  ...
  console.log(`[carga] ${r.coordenadores} coordenadores carregados.`);
  if (r.semTelefone > 0) {
