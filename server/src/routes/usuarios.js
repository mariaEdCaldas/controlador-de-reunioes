...
const contarAdmins = async () =>
  (await db.prepare("SELECT COUNT(*) AS n FROM usuarios WHERE papel = 'admin'").get()).n;

/** GET /api/usuarios */
usuariosRouter.get('/', async (req, res) => {
  res.json(await db.prepare('SELECT id, nome, email, papel, criado_em FROM usuarios ORDER BY nome COLLATE NOCASE').all());
});
...
usuariosRouter.post('/', async (req, res) => {
  ...
  if (await db.prepare('SELECT 1 FROM usuarios WHERE email = ? COLLATE NOCASE').get(email)) {
  ...
  res.status(201).json(publico(await db.prepare('SELECT * FROM usuarios WHERE id = ?').get(lastInsertRowid)));
});
...
usuariosRouter.patch('/:id', async (req, res) => {
  ...
  if (u.papel === 'admin' && papel !== 'admin' && (await contarAdmins()) <= 1) {
  ...
  await db.prepare('UPDATE usuarios SET nome = ?, papel = ?, senha_hash = ? WHERE id = ?')
    .run(nome, papel, hash, u.id);
  res.json(publico(await db.prepare('SELECT * FROM usuarios WHERE id = ?').get(u.id)));
});
...
usuariosRouter.delete('/:id', async (req, res) => {
  ...
  if (u.papel === 'admin' && (await contarAdmins()) <= 1) {
    return res.status(400).json({ erro: 'Não dá para excluir o único administrador.' });
  }
  await db.prepare('DELETE FROM usuarios WHERE id = ?').run(u.id);
  res.json({ ok: true });
});
